function market_simulation()
    ...
    %資産変動を.txtに保存
    %writematrix(capital_changes', sprintf('capital_changes_B%d.csv', B));

    %プレイヤー数をCSVに保存/プロット
    writematrix(playerCounts, sprintf('player_counts_B%d.csv', B));
    figure;
    plot(1:iteration, playerCounts);
    xlabel('t');
    ylabel('The number of players');
    %grid on;
    exportgraphics(gcf, sprintf('player_counts_B%d.pdf', B));

    %リターンをCSVに保存/プロット
    writematrix(returns, sprintf('return_B%d.csv', B));
    figure;
    plot(returns);
    xlabel('t');
    ylabel('r(t)');
    %grid on;
    exportgraphics(gcf, sprintf('return_B%d.pdf', B));
end

function market_sosg_ising_2(n)
    ...
    for t = 1:iteration
        ...
        if N_current == 0
            % 新規参入
            [players, occupied] = enter_market(players, occupied, B, pattern, b_imax, gridN);
            returns(t) = 0;
            continue;
        end
        ...
        if N_current > 0
            ...
            for i = 1:N_current
                Es(i) = local_expect_spin(players, i, gridN);
            end
...
function Es = local_expect_spin(players, i, n)
    position = players{i}.pos; 
    Es_sum = 0; 
    cnt = 0;
    nbrs = [position+[ -1 0]; position+[1 0]; position+[0 -1]; position+[0 1]];
    for k=1:4
        x=nbrs(k,1);
        y=nbrs(k,2);
        if x>=1 && x<=n && y>=1 && y<=n
...
%% === 新規プレイヤー参入 ===
function [players, occupied] = enter_market(players, occupied, B, pattern, b_imax, gridN)
    % 格子の空きを探す
    ...
    if ~isempty(available_indices)
        ...
        players{end+1} = create_player(B, pattern, position, b_imax);
